...
for i in $(seq 1 120); do
  if php -r "try { new PDO('mysql:host=${DB_HOST};port=${DB_PORT};dbname=${DB_DATABASE}', '${DB_USERNAME}', '${DB_PASSWORD}', [PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION]); } catch (Throwable \$e) { exit(1); }" >/dev/null 2>&1; then
    DB_OK=1
    echo "[entrypoint] MySQL conectado apos ${i}s."
    break
  fi
  if [ $((i % 10)) -eq 0 ]; then
    echo "[entrypoint] Aguardando MySQL... (${i}/120)"
  fi
  sleep 1
done

if [ "$DB_OK" -ne 1 ]; then
  echo "[entrypoint] ERRO: MySQL indisponivel apos 120s."
  echo "[entrypoint] Verifique:"
  echo "  - DB_HOST=${DB_HOST}"
  echo "  - DB_PORT=${DB_PORT}"
  echo "  - DB_DATABASE=${DB_DATABASE}"
  echo "  - DB_USERNAME=${DB_USERNAME}"
  echo "  - Se o servico MySQL esta rodando e acessivel"
  exit 1
fi
